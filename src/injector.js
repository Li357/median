...
async function processMath(rawMath, fontSize = 30, fontColor = '000000') {
  const [uri, width, height] = await createMathImage(rawMath, fontSize, fontColor);
  const file = await createMathImageFile(uri);
  placeFileIntoPost(file);
}
